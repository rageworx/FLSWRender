...
namespace ____MATH__Matrix
{
	class Matrix
	{
        ...
        public:
            ...
            :m(std::vector<std::vector<float>>(row,std::vector<float>(col,0.f))),ncols(col),nrows(row){ }
            Matrix(){ }
            
            // Column matrix 4*1 
            Matrix(vec3f v)
            :m(std::vector<std::vector<float> >(4, std::vector<float>(1, 1.f))),nrows(4), ncols(1)
            {
                m[0][0] = v.x;
                m[1][0] = v.y;
                m[2][0] = v.z;
                m[3][0] = 1.f; /// The w component is set to 0.0f 
            }
        ...
        public:
            ...
            Matrix transpose();//¾ØÕó×ªÖÃ
            Matrix inverse();

            // The 4x1 matrix is converted to a vector, and the w component is lost 
